	...
	for opt,arg in opts:
		if opt in ("-a","--articles"):
			...
			with open(arg,'rb') as f:
				text = f.read().decode()
				articles.extend(re.sub(r'[^\w/\.\-\_]+',' ',text).split())
...
def train():
	...
	articles = []
	with open("/Users/christian/Desktop/cbmi/reproduce/python/articles/sub_pmc_result.xml",'r') as pc:
		bs = BeautifulSoup(pc.read(),'lxml')
	bs_articles = list(bs.find_all('article-id',{'pub-id-type':'doi'}))
	list_articles = list(map(lambda x: x.text.strip(),bs_articles))
	print(len(list_articles))

	#print(articles.index('10.1186/s12911-016-0254-y'))
	#articles = articles[:articles.index('10.1186/s12911-016-0254-y')]
	Trainer('inclusion_criteria',list_articles)
